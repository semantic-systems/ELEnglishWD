...
One has to download n-triples files of the different KGs and the json dump of Wikidata.
...
and [calculate_mention_overlap_plus_mention_dictionary.py](kg_evaluation/scripts/calculate_mention_overlap_plus_mention_dictionary.py) exists. 
Both expect a n-triples file. The main difference is that calculate_mention_overlap_wikidata.py also creates a json file which
...
mean, median or other percentile length.
